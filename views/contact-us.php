    ...
    <div class="container">
        <div class="row align-items-center justify-content-center h-500px sm-h-300px">
            <div class="col-12 col-md-6 position-relative text-center page-title-extra-large d-flex flex-wrap flex-column align-items-center justify-content-center" data-anime='{ "el": "childs", "translateY": [30, 0], "opacity": [0,1], "duration": 600, "delay": 0, "staggervalue": 300, "easing": "easeOutQuad" }'>
                <span class="ps-25px pe-25px pt-5px pb-5px mb-15px text-uppercase text-white fs-12 ls-1px fw-600 border-radius-100px bg-gradient-dark-gray-transparent d-flex"><i class="bi bi-megaphone text-white icon-small me-10px"></i>Grow your IT Infrastructure with us</span>
                    ...
                    <div class="col services-box-style-02 mb-30px">
                        <div class="row g-0 box-shadow-quadruple-large border-radius-6px overflow-hidden">
                            <div class="col-lg-6 col-sm-6">
                                <div class="h-100 cover-background xs-h-300px" style="background-image: url('/public/images/common/nilamber-triumph-drawing.webp')"></div>
                            </div>
...
<script>
    ...
    document.addEventListener("ajaxFormSubmitResponse", function(event) {
        ...
        if (!response || !response.alert || !response.message) return;

        // Remove existing alert (if any)
        const oldAlert = document.querySelector(".custom-form-alert");
        if (oldAlert) oldAlert.remove();

        // Create new alert div
        const alertDiv = document.createElement("div");
        alertDiv.className = `alert ${response.alert} custom-form-alert alert-dismissible fade show mt-4`;
        alertDiv.role = "alert";
        alertDiv.innerHTML = `
        ${response.message}
    ...
    `;
        ...
        if (response.alert.includes("alert-success") && form) {
            form.reset(); // Clears all standard inputs, textareas, etc.

            // Optional: manually clear any weird custom fields if needed
