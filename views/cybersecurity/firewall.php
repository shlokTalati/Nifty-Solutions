<?php $page_title = "Network Firewall Services in Vadodara | Nifty Solutions";
$meta_description = "Nifty Solutions provides professional network firewall services in Vadodara for small and medium businesses. We offer Fortinet and Sophos firewall solutions with VPN, threat protection, and ongoing IT support across Gujarat.";?>

<!-- start page title -->
<section class="pt-0 cover-background ipad-top-space-margin sm-pb-0">
    <img src="/public/images/firewall/firewall-inner.webp"
         alt="Network Firewall Services Vadodara"
         class="position-absolute top-0 start-0 w-100 h-100 object-fit-cover z-index-minus-1"
         fetchpriority="high"
         decoding="async"
         loading="eager" />
    <div class="shape-image-animation bottom-0 p-0 w-100 d-none d-md-block">
        <svg xmlns="http://www.w3.org/2000/svg" width="3000" height="400" viewBox="0 180 2500 200" fill="#ffffff">
            <path class="st1" d="M 0 250 C 1200 400 1200 50 3000 250 L 3000 550 L 0 550 L 0 250">
                <animate
                    attributeName="d"
                    dur="5s"
                    values="M 0 250 C 1200 400 1200 50 3000 250 L 3000 550 L 0 550 L 0 250;
                            M 0 250 C 400 50 400 400 3000 250 L 3000 550 L 0 550 L 0 250;
                            M 0 250 C 1200 400 1200 50 3000 250 L 3000 550 L 0 550 L 0 250"
                    repeatCount="indefinite" />
            </path>
        </svg>
    </div>
    <div class="container">
        <div class="row align-items-center justify-content-center h-500px sm-h-300px">
            <div class="col-12 col-lg-8 col-md-10 position-relative text-center page-title-extra-large d-flex flex-wrap flex-column align-items-center justify-content-center" data-anime='{ "el": "childs", "translateY": [30, 0], "opacity": [0,1], "duration": 600, "delay": 0, "staggervalue": 300, "easing": "easeOutQuad" }'>
                <span class="ps-25px pe-25px pt-5px pb-5px mb-15px text-uppercase text-white fs-12 ls-1px fw-600 border-radius-100px bg-gradient-dark-gray-transparent d-flex"><i class="bi bi-megaphone text-white icon-small me-10px"></i>Security</span>
                <h1 class="mb-0 text-white fw-600 ls-minus-1px">Firewall Services to Protect Your Business Network</h1>
            </div>
        </div>
    </div>
</section>
<!-- end page title -->

<!-- start section -->
<section class="pt-3 sm-pt-50px">
    <div class="container">
        <div class="row align-items-center justify-content-center" data-anime='{ "perspective": 1200 }'>
            <div class="col-lg-6 md-mb-50px" data-anime='{ "translateY": [0, 0], "zoom": [1.1, 1], "rotateX": [50, 0], "opacity": [0,1], "duration": 800, "delay": 200, "staggervalue": 300, "easing": "easeOutQuad" }'>
                <figure class="position-relative m-0">
                    <img src="/public/images/firewall/firewall1.webp" alt="Network Firewall Services Vadodara" class="w-100 border-radius-5px" data-anime='{ "translateY": [0, 0], "opacity": [0,1], "duration": 800, "delay": 0, "staggervalue": 150, "easing": "easeOutQuad" }'>
                </figure>
            </div>
            <div class="col-xl-5 offset-xl-1 col-lg-6 text-center text-md-start" data-anime='{ "el": "childs", "translateY": [30, 0], "opacity": [0,1], "duration": 600, "delay": 0, "staggervalue": 300, "easing": "easeOutQuad" }'>
                <span class="ps-25px pe-25px mb-20px text-uppercase text-base-color fs-12 lh-40 fw-700 border-radius-100px bg-gradient-very-light-gray-transparent d-inline-flex justify-content-center justify-content-md-start"><i class="bi bi-award fs-16 me-5px"></i>Advanced Firewall</span>
                <h2 class="text-dark-gray fw-700 ls-minus-1px">Firewall Devices and Solutions</h2>
                <p class="w-95 md-w-100">Businesses today rely heavily on the internet for communication, operations, and data management. However, constant connectivity also exposes company networks to cyber threats, malware, and unauthorized access. Network firewall services act as the first line of defense for your business network — monitoring and controlling incoming and outgoing traffic to ensure only safe, authorized connections are allowed.</p>
                <p class="w-95 md-w-100">At Nifty Solutions, we provide reliable network firewall services in Vadodara designed to protect small and medium businesses that may not have an in-house IT team. Our solutions help offices, manufacturing companies, clinics, and service businesses secure their networks while maintaining smooth day-to-day operations. As a trusted IT service provider in Vadodara with over 25 years of experience, we help businesses build strong security systems that prevent downtime, data breaches, and costly cyber incidents.</p>
                <div class="pt-20px pb-20px ps-30px pe-30px xs-p-15px border border-color-extra-medium-gray border-radius-6px mb-15px icon-with-text-style-08 w-90 lg-w-100">
                    <div class="feature-box feature-box-left-icon-middle d-inline-flex align-middle">
                        <div class="feature-box-icon me-15px">
                            <i class="bi bi-people icon-very-medium text-base-color"></i>
                        </div>
                    </div>
                    <p class="fs-13 mb-0">Based in Vadodara. Serving businesses <span class="text-dark-gray text-decoration-line-bottom">across</span> Gujarat and India</p>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- end section -->

<!-- start section - Common Problems -->
<section class="bg-very-light-gray pt-5 pb-5">
    <div class="container">
        <div class="row justify-content-center mb-4">
            <div class="col-xl-8 col-lg-9 col-md-10 text-center" data-anime='{ "el": "childs", "translateY": [30, 0], "opacity": [0,1], "duration": 600, "delay": 0, "staggervalue": 300, "easing": "easeOutQuad" }'>
                <span class="ps-25px pe-25px mb-20px text-uppercase text-base-color fs-12 lh-40 fw-700 border-radius-100px bg-gradient-very-light-gray-transparent d-inline-flex justify-content-center"><i class="bi bi-exclamation-triangle fs-16 me-5px"></i>Why It Matters</span>
                <h2 class="text-dark-gray fw-700 ls-minus-1px">Common Problems Businesses Face Without a Firewall</h2>
                <p class="w-80 md-w-100 mx-auto">Many businesses operate with basic routers or outdated security systems. Without proper firewall protection, this can create serious risks for your operations and data.</p>
            </div>
        </div>
        <div class="row row-cols-1 row-cols-lg-3 row-cols-md-2 justify-content-center" data-anime='{ "el": "childs", "translateY": [30, 0], "opacity": [0,1], "duration": 600, "delay": 0, "staggervalue": 200, "easing": "easeOutQuad" }'>
            <!-- item -->
            <div class="col mb-30px">
                <div class="bg-white border-radius-6px box-shadow-extra-large p-35px h-100">
                    <div class="feature-box feature-box-left-icon-middle overflow-hidden mb-15px">
                        <div class="feature-box-icon feature-box-icon-rounded w-45px h-45px bg-gradient-very-light-gray-transparent rounded-circle me-15px">
                            <i class="bi bi-shield-x fs-18 text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-600 fs-16">Unauthorized Access</span>
                        </div>
                    </div>
                    <p class="mb-0 fs-15">Hackers and malicious users can gain access to your company systems, putting sensitive data and operations at risk.</p>
                </div>
            </div>
            <!-- item -->
            <div class="col mb-30px">
                <div class="bg-white border-radius-6px box-shadow-extra-large p-35px h-100">
                    <div class="feature-box feature-box-left-icon-middle overflow-hidden mb-15px">
                        <div class="feature-box-icon feature-box-icon-rounded w-45px h-45px bg-gradient-very-light-gray-transparent rounded-circle me-15px">
                            <i class="bi bi-bug fs-18 text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-600 fs-16">Malware and Ransomware</span>
                        </div>
                    </div>
                    <p class="mb-0 fs-15">Malware and ransomware attacks can damage critical files or lock company data, leading to costly downtime and recovery.</p>
                </div>
            </div>
            <!-- item -->
            <div class="col mb-30px">
                <div class="bg-white border-radius-6px box-shadow-extra-large p-35px h-100">
                    <div class="feature-box feature-box-left-icon-middle overflow-hidden mb-15px">
                        <div class="feature-box-icon feature-box-icon-rounded w-45px h-45px bg-gradient-very-light-gray-transparent rounded-circle me-15px">
                            <i class="bi bi-globe2 fs-18 text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-600 fs-16">Unsafe Website Access</span>
                        </div>
                    </div>
                    <p class="mb-0 fs-15">Employees accessing unsafe or non-work websites can expose the entire network to threats and reduce overall productivity.</p>
                </div>
            </div>
            <!-- item -->
            <div class="col mb-30px">
                <div class="bg-white border-radius-6px box-shadow-extra-large p-35px h-100">
                    <div class="feature-box feature-box-left-icon-middle overflow-hidden mb-15px">
                        <div class="feature-box-icon feature-box-icon-rounded w-45px h-45px bg-gradient-very-light-gray-transparent rounded-circle me-15px">
                            <i class="bi bi-wifi-off fs-18 text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-600 fs-16">Network Downtime</span>
                        </div>
                    </div>
                    <p class="mb-0 fs-15">Cyber attacks and suspicious traffic can overwhelm your network, causing outages that interrupt business operations.</p>
                </div>
            </div>
            <!-- item -->
            <div class="col mb-30px">
                <div class="bg-white border-radius-6px box-shadow-extra-large p-35px h-100">
                    <div class="feature-box feature-box-left-icon-middle overflow-hidden mb-15px">
                        <div class="feature-box-icon feature-box-icon-rounded w-45px h-45px bg-gradient-very-light-gray-transparent rounded-circle me-15px">
                            <i class="bi bi-database-x fs-18 text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-600 fs-16">Data Theft and Leaks</span>
                        </div>
                    </div>
                    <p class="mb-0 fs-15">Sensitive customer and financial information can be stolen or leaked, causing serious legal, financial, and reputational damage.</p>
                </div>
            </div>
            <!-- item -->
            <div class="col mb-30px">
                <div class="bg-white border-radius-6px box-shadow-extra-large p-35px h-100">
                    <div class="feature-box feature-box-left-icon-middle overflow-hidden mb-15px">
                        <div class="feature-box-icon feature-box-icon-rounded w-45px h-45px bg-gradient-very-light-gray-transparent rounded-circle me-15px">
                            <i class="bi bi-person-x fs-18 text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-600 fs-16">Lost Productivity</span>
                        </div>
                    </div>
                    <p class="mb-0 fs-15">Uncontrolled internet usage and unmanaged network access reduce employee focus and drag down overall business performance.</p>
                </div>
            </div>
        </div>
    </div>
    <hr>
</section>
<!-- end section -->
<!-- start section - Featured Solutions (existing rotate box section preserved) -->
<section class="bg-very-light-gray">
    <div class="container">
        <div class="row justify-content-center mb-3">
            <div class="col-xl-8 col-lg-9 col-md-10 text-center">
                <h2 class="text-dark-gray fw-700 ls-minus-1px" data-anime='{ "translateY": [30, 0], "opacity": [0,1], "duration": 600, "delay": 0, "staggervalue": 300, "easing": "easeOutQuad" }'>Featured Solutions</h2>
            </div>
        </div>
        <div class="row row-cols-1 row-cols-lg-4 row-cols-sm-2 justify-content-center" data-anime='{ "el": "childs", "willchange": "transform", "scale":[0.95,1], "opacity": [0, 1], "duration": 800, "delay": 100, "staggervalue": 300, "easing": "easeOutQuad" }'>
            <!-- start rotate box item -->
            <div class="col rotate-box-style-02 mb-30px">
                <div class="w-100 min-h-300px text-center rotate-box to-left">
                    <div class="w-100 h-100 overflow-hidden z-index-1 front-side bg-white border-radius-6px box-shadow-quadruple-large">
                        <div class="rotate-content-front z-index-2 p-30px">
                            <div class="fs-19 text-dark-gray alt-font fw-600 mb-5px">Sophos</div>
                            <span>Sophos Firewall protects your network with AI-based threat detection and secure VPN features — simple, fast, and effective for small and medium businesses.</span>
                        </div>
                    </div>
                    <div class="w-100 h-100 overflow-hidden back-side cover-background border-radius-6px box-shadow-quadruple-large" style="background-image:url('/public/images/firewall/sophos.webp')">
                        <div class="opacity-light bg-charcoal-blue"></div>
                        <div class="d-flex flex-column align-items-center justify-content-center h-100 z-index-2 rotate-content-back p-30px">
                            </br></br></br></br></br></br>
                            <a href="https://www.sophos.com/en-us/products/next-gen-firewall" target="_blank" class="btn btn-medium btn-rounded with-rounded btn-base-color btn-box-shadow">Learn more<span class="bg-white text-base-color"><i class="feather icon-feather-arrow-right"></i></span></a>
                        </div>
                    </div>
                </div>
            </div>
            <!-- end rotate box item -->
            <!-- start rotate box item -->
            <div class="col rotate-box-style-02 mb-30px">
                <div class="w-100 min-h-300px text-center rotate-box to-left">
                    <div class="w-100 h-100 overflow-hidden z-index-1 front-side bg-white border-radius-6px box-shadow-quadruple-large">
                        <div class="rotate-content-front z-index-2 p-30px">
                            <div class="fs-19 text-dark-gray alt-font fw-600 mb-5px">Fortinet</div>
                            <span>Fortinet FortiGate offers high-speed next-generation firewall security with built-in SD-WAN and real-time threat protection from FortiGuard Labs.</span>
                        </div>
                    </div>
                    <div class="w-100 h-100 overflow-hidden back-side cover-background border-radius-6px box-shadow-quadruple-large" style="background-image:url('/public/images/firewall/fortinet.webp')">
                        <div class="opacity-light bg-charcoal-blue"></div>
                        <div class="d-flex flex-column align-items-center justify-content-center h-100 z-index-2 rotate-content-back p-30px">
                            </br></br></br></br></br></br>
                            <a href="https://www.fortinet.com/products/next-generation-firewall" target="_blank" class="btn btn-medium btn-rounded with-rounded btn-base-color btn-box-shadow">Learn more<span class="bg-white text-base-color"><i class="feather icon-feather-arrow-right"></i></span></a>
                        </div>
                    </div>
                </div>
            </div>
            <!-- end rotate box item -->
        </div>
    </div>
</section>
<!-- end section -->

<!-- start section - Benefits + Image (existing preserved) -->
<section>
    <div class="container">
        <div class="row align-items-center justify-content-center mb-7 sm-mb-40px">
            <div class="col-xl-5 col-lg-6 col-md-12 md-mb-50px" data-anime='{ "el": "childs", "translateY": [15, 0], "opacity": [0,1], "duration": 600, "delay": 0, "staggervalue": 100, "easing": "easeOutQuad" }'>
                <span class="ps-25px pe-25px mb-20px text-uppercase text-base-color fs-12 lh-40 fw-700 border-radius-100px bg-gradient-very-light-gray-transparent d-inline-flex"><i class="bi bi-patch-check fs-16 me-5px"></i>Services benefits</span>
                <h2 class="fw-700 text-dark-gray ls-minus-1px">Benefits of Using a Firewall for Your Business</h2>
                <!-- start features box item -->
                <div class="icon-with-text-style-08 mb-10px">
                    <div class="feature-box feature-box-left-icon-middle overflow-hidden">
                        <div class="feature-box-icon feature-box-icon-rounded w-35px h-35px bg-very-light-gray rounded-circle me-15px">
                            <i class="fa-solid fa-check fs-12 text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-500">Block internet access on network computers or provide limited website access by user or category</span>
                        </div>
                    </div>
                </div>
                <!-- end features box item -->
                <!-- start features box item -->
                <div class="icon-with-text-style-08 mb-10px">
                    <div class="feature-box feature-box-left-icon-middle overflow-hidden">
                        <div class="feature-box-icon feature-box-icon-rounded w-35px h-35px bg-very-light-gray rounded-circle me-15px">
                            <i class="fa-solid fa-check fs-12 text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-500">Provide secure access to your office network from anywhere using Virtual Private Network (VPN)</span>
                        </div>
                    </div>
                </div>
                <!-- end features box item -->
                <!-- start features box item -->
                <div class="icon-with-text-style-08 mb-10px">
                    <div class="feature-box feature-box-left-icon-middle overflow-hidden">
                        <div class="feature-box-icon feature-box-icon-rounded w-35px h-35px bg-very-light-gray rounded-circle me-15px">
                            <i class="fa-solid fa-check fs-12 text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-500">Protects against hackers, malware, and ransomware attacks that can disrupt business operations</span>
                        </div>
                    </div>
                </div>
                <!-- end features box item -->
                <!-- start features box item -->
                <div class="icon-with-text-style-08 mb-10px">
                    <div class="feature-box feature-box-left-icon-middle overflow-hidden">
                        <div class="feature-box-icon feature-box-icon-rounded w-35px h-35px bg-very-light-gray rounded-circle me-15px">
                            <i class="fa-solid fa-check fs-12 text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-500">Enhances privacy and data security for customer and financial information</span>
                        </div>
                    </div>
                </div>
                <!-- end features box item -->
                <!-- start features box item -->
                <div class="icon-with-text-style-08">
                    <div class="feature-box feature-box-left-icon-middle overflow-hidden">
                        <div class="feature-box-icon feature-box-icon-rounded w-35px h-35px bg-very-light-gray rounded-circle me-15px">
                            <i class="fa-solid fa-check fs-12 text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-500">Supports compliance with industry security standards and regulatory requirements</span>
                        </div>
                    </div>
                </div>
                <!-- end features box item -->
            </div>
            <div class="col-lg-6 offset-xl-1 position-relative z-index-1" data-anime='{ "opacity": [0,1], "duration": 600, "delay": 100, "staggervalue": 300, "easing": "easeOutQuad" }'>
                <div class="atropos" data-atropos data-atropos-perspective="2450">
                    <div class="atropos-scale">
                        <div class="atropos-rotate">
                            <div class="atropos-inner">
                                <img src="/public/images/firewall/firewall-draw.webp" alt="Firewall Solutions Vadodara" class="border-radius-6px w-100" />
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <!-- Prominent Highlights Accordion (existing preserved + SEO content updated) -->
        <div class="row align-items-center mt-8 sm-mt-40px" data-anime='{ "translateY": [0, 0], "opacity": [0,1], "duration": 600, "delay": 0, "staggervalue": 300, "easing": "easeOutQuad" }'>
            <div class="col-12">
                <div class="bg-very-light-gray p-9 md-p-6 xs-p-9 border-radius-6px overflow-hidden position-relative">
                    <div class="position-absolute right-70px lg-right-20px top-minus-20px w-250px sm-w-180px xs-w-150px opacity-1"><img src="/public/images/firewall/firewall-icon.webp" alt="Firewall Icon"></div>
                    <span class="ps-25px pe-25px mb-20px text-uppercase text-base-color fs-12 lh-40 fw-700 border-radius-100px bg-gradient-light-gray-transparent d-inline-flex"><i class="bi bi-patch-check fs-16 me-5px"></i>Basic information of Firewall</span>
                    <h2 class="fw-700 text-dark-gray ls-minus-1px">Prominent Highlights</h2>
                    <div class="accordion accordion-style-02" id="accordion-style-02" data-active-icon="icon-feather-minus" data-inactive-icon="icon-feather-plus">

                        <!-- start accordion item -->
                        <div class="accordion-item">
                            <div class="accordion-header border-bottom border-color-transparent-dark-very-light">
                                <a href="#" data-bs-toggle="collapse" data-bs-target="#accordion-style-02-01" aria-expanded="false" data-bs-parent="#accordion-style-02">
                                    <div class="accordion-title mb-0 position-relative text-dark-gray pe-30px">
                                        <i class="feather icon-feather-plus fs-20"></i><span class="fs-17 fw-500">Fortinet Firewall (FortiGate)</span>
                                    </div>
                                </a>
                            </div>
                            <div id="accordion-style-02-01" class="accordion-collapse collapse" data-bs-parent="#accordion-style-02">
                                <div class="accordion-body last-paragraph-no-margin border-bottom border-color-transparent-dark-very-light">
                                    <p>Fortinet FortiGate is one of the most advanced next-generation firewall (NGFW) solutions used by businesses worldwide. It is designed to provide high-performance, integrated network security for enterprises, data centres, cloud environments, and small businesses — powered by AI, machine learning, and real-time threat intelligence from FortiGuard Labs.</p>
                                    <p><strong>Key capabilities include:</strong></p>
                                    <ul>
                                        <li><strong>Deep Packet Inspection</strong> — Analyzes network traffic in detail to detect and block hidden threats before they reach your systems.</li>
                                        <li><strong>Intrusion Prevention System (IPS)</strong> — Continuously monitors and blocks suspicious activity on your network in real time.</li>
                                        <li><strong>Application Control</strong> — Manages which applications are allowed or restricted on your business network.</li>
                                        <li><strong>Secure VPN Connectivity</strong> — Enables remote employees or branch offices to securely connect to the main office network from anywhere.</li>
                                        <li><strong>Integrated SD-WAN</strong> — Provides efficient network management across multiple locations with centralized control via FortiManager or FortiCloud.</li>
                                        <li><strong>Real-Time Threat Intelligence</strong> — Constant threat updates from Fortinet's in-house research team, FortiGuard Labs, ensure your network stays protected against the latest threats.</li>
                                    </ul>
                                    <p>FortiGate firewalls are ideal for companies that need strong, high-speed security without compromising network performance.</p>
                                </div>
                            </div>
                        </div>
                        <!-- end accordion item -->

                        <!-- start accordion item -->
                        <div class="accordion-item">
                            <div class="accordion-header border-bottom border-color-transparent-dark-very-light">
                                <a href="#" data-bs-toggle="collapse" data-bs-target="#accordion-style-02-02" aria-expanded="false" data-bs-parent="#accordion-style-02">
                                    <div class="accordion-title mb-0 position-relative text-dark-gray pe-30px">
                                        <i class="feather icon-feather-plus fs-20"></i><span class="fs-17 fw-500">Sophos Firewall</span>
                                    </div>
                                </a>
                            </div>
                            <div id="accordion-style-02-02" class="accordion-collapse collapse" data-bs-parent="#accordion-style-02">
                                <div class="accordion-body last-paragraph-no-margin border-bottom border-color-transparent-dark-very-light">
                                    <p>Sophos is a global leader in cybersecurity, providing intelligent protection using AI-based threat detection and advanced network management features. Sophos firewalls are especially effective for small and medium businesses that need powerful security with simple, easy-to-manage tools.</p>
                                    <p><strong>Key features include:</strong></p>
                                    <ul>
                                        <li><strong>Website and Content Filtering</strong> — Blocks unsafe, non-work, or unauthorized websites based on categories, keywords, or user-specific rules to keep employees focused and the network safe.</li>
                                        <li><strong>Gateway Antivirus Protection</strong> — Scans all incoming and outgoing internet traffic for malware, viruses, and spam, ensuring a clean and secure network.</li>
                                        <li><strong>Bandwidth Management</strong> — Allocates bandwidth based on business priorities, so critical applications like ERP, emails, and video calls always perform well.</li>
                                        <li><strong>Multi-Internet Link Management</strong> — Supports multiple internet connections with automatic failover, ensuring uninterrupted business connectivity even if one connection fails.</li>
                                        <li><strong>VPN Access</strong> — Allows employees to securely connect to office systems, applications such as Tally and ERP, and company data from home or while travelling.</li>
                                    </ul>
                                    <p>Sophos firewalls provide the right balance of advanced protection and ease of management, making them an excellent choice for growing businesses in Vadodara and Gujarat.</p>
                                </div>
                            </div>
                        </div>
                        <!-- end accordion item -->

                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- end section -->

<!-- start section - Our Firewall Offering (existing preserved) -->
<section class="bg-very-light-gray overlap-height position-relative">
    <div class="container overlap-gap-section">
        <div class="row justify-content-center mb-4">
            <div class="col-xl-7 col-lg-9 col-md-10 text-center">
                <h2 class="text-dark-gray fw-700 ls-minus-2px" data-anime='{ "translateY": [30, 0], "opacity": [0,1], "duration": 600, "delay": 0, "staggervalue": 300, "easing": "easeOutQuad" }'>Our Firewall Offering</h2>
            </div>
        </div>
        <div class="row row-cols-1 row-cols-lg-3 row-cols-md-2 justify-content-center mb-8" data-anime='{ "el": "childs", "translateY": [50, 0], "translateX": [-30, 0], "opacity": [0,1], "duration": 600, "delay": 0, "staggervalue": 300, "easing": "easeOutQuad" }'>
            <!-- start services box style -->
            <div class="col md-mb-30px">
                <div class="services-box-style-03 box-shadow-extra-large last-paragraph-no-margin border-radius-6px overflow-hidden">
                    <div class="position-relative">
                        <img src="/public/images/firewall/firewall-fortinet.webp" alt="Fortinet Firewall Vadodara">
                        <span class="btn btn-very-small btn-rounded btn-dark-gray text-white btn-box-shadow ps-15px pe-15px pt-5px pb-5px lh-16 position-absolute right-30px top-30px text-uppercase">Fortinet</span>
                    </div>
                    <div class="bg-white">
                        <div class="ps-65px pe-65px lg-ps-30px lg-pe-30px pt-30px pb-30px text-center">
                            <strong class="fw-700 text-dark-gray">Fortinet Firewall</strong>
                        </div>
                        <div class="d-flex justify-content-center border-top border-color-extra-medium-gray pt-20px pb-20px ps-50px pe-50px position-relative text-center">
                            <a href="https://www.fortinet.com/products/next-generation-firewall" target="_blank" rel="nofollow" class="btn btn-link btn-hover-animation-switch btn-medium fw-700 text-dark-gray text-uppercase">
                                <span>
                                    <span class="btn-text">More Information</span>
                                    <span class="btn-icon"><i class="fa-solid fa-arrow-right"></i></span>
                                    <span class="btn-icon"><i class="fa-solid fa-arrow-right"></i></span>
                                </span>
                            </a>
                        </div>
                    </div>
                </div>
            </div>
            <!-- end services box style -->
            <!-- start services box style -->
            <div class="col md-mb-30px">
                <div class="services-box-style-03 box-shadow-extra-large last-paragraph-no-margin border-radius-6px overflow-hidden">
                    <div class="position-relative">
                        <img src="/public/images/firewall/firewall-sophos.webp" alt="Sophos Firewall Vadodara">
                        <span class="btn btn-very-small btn-rounded btn-dark-gray text-white btn-box-shadow ps-15px pe-15px pt-5px pb-5px lh-16 position-absolute right-30px top-30px text-uppercase">Sophos</span>
                    </div>
                    <div class="bg-white">
                        <div class="ps-65px pe-65px lg-ps-30px lg-pe-30px pt-30px pb-30px text-center">
                            <strong class="fw-700 text-dark-gray">Sophos Firewall</strong>
                        </div>
                        <div class="d-flex justify-content-center border-top border-color-extra-medium-gray pt-20px pb-20px ps-50px pe-50px position-relative text-center">
                            <a href="https://www.sophos.com/en-us/products/next-gen-firewall" target="_blank" rel="nofollow" class="btn btn-link btn-hover-animation-switch btn-medium fw-700 text-dark-gray text-uppercase">
                                <span>
                                    <span class="btn-text">More Information</span>
                                    <span class="btn-icon"><i class="fa-solid fa-arrow-right"></i></span>
                                    <span class="btn-icon"><i class="fa-solid fa-arrow-right"></i></span>
                                </span>
                            </a>
                        </div>
                    </div>
                </div>
            </div>
            <!-- end services box style -->
        </div>
    </div>
    <div class="shape-image-animation p-0 w-100 bottom-minus-40px xl-bottom-0px d-none d-md-block">
        <svg xmlns="http://www.w3.org/2000/svg" width="3000" height="400" viewBox="0 180 2500 200" fill="#ffffff">
            <path class="st1" d="M 0 250 C 1200 400 1200 50 3000 250 L 3000 550 L 0 550 L 0 250">
                <animate
                    attributeName="d"
                    dur="5s"
                    values="M 0 250 C 1200 400 1200 50 3000 250 L 3000 550 L 0 550 L 0 250;
                            M 0 250 C 400 50 400 400 3000 250 L 3000 550 L 0 550 L 0 250;
                            M 0 250 C 1200 400 1200 50 3000 250 L 3000 550 L 0 550 L 0 250"
                    repeatCount="indefinite" />
            </path>
        </svg>
    </div>
</section>
<!-- end section -->

<!-- start section - Why Choose Nifty Solutions -->
<section class="pt-5 pb-5">
    <div class="container">
        <div class="row justify-content-center mb-4">
            <div class="col-xl-8 col-lg-9 col-md-10 text-center" data-anime='{ "el": "childs", "translateY": [30, 0], "opacity": [0,1], "duration": 600, "delay": 0, "staggervalue": 300, "easing": "easeOutQuad" }'>
                <span class="ps-25px pe-25px mb-20px text-uppercase text-base-color fs-12 lh-40 fw-700 border-radius-100px bg-gradient-very-light-gray-transparent d-inline-flex justify-content-center"><i class="bi bi-building fs-16 me-5px"></i>Why Choose Us</span>
                <h2 class="text-dark-gray fw-700 ls-minus-1px">Why Choose Nifty Solutions as Your IT Service Provider</h2>
                <p class="w-80 md-w-100 mx-auto">For over 25 years, Nifty Solutions has been helping businesses in Vadodara and Gujarat build secure and reliable IT infrastructure. When you work with us, you get more than just hardware installation — you get a dedicated managed IT services partner who understands your business.</p>
            </div>
        </div>
        <div class="row align-items-center justify-content-center" data-anime='{ "el": "childs", "translateY": [30, 0], "opacity": [0,1], "duration": 600, "delay": 0, "staggervalue": 200, "easing": "easeOutQuad" }'>
            <div class="col-xl-5 col-lg-6 col-md-12 md-mb-50px">
                <!-- start features box item -->
                <div class="icon-with-text-style-08 mb-15px">
                    <div class="feature-box feature-box-left-icon-middle overflow-hidden">
                        <div class="feature-box-icon feature-box-icon-rounded w-35px h-35px bg-very-light-gray rounded-circle me-15px">
                            <i class="fa-solid fa-check fs-12 text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-500">Professional firewall selection based on your specific business size and requirements</span>
                        </div>
                    </div>
                </div>
                <!-- end features box item -->
                <!-- start features box item -->
                <div class="icon-with-text-style-08 mb-15px">
                    <div class="feature-box feature-box-left-icon-middle overflow-hidden">
                        <div class="feature-box-icon feature-box-icon-rounded w-35px h-35px bg-very-light-gray rounded-circle me-15px">
                            <i class="fa-solid fa-check fs-12 text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-500">Secure firewall configuration, deployment, and testing by experienced IT professionals</span>
                        </div>
                    </div>
                </div>
                <!-- end features box item -->
                <!-- start features box item -->
                <div class="icon-with-text-style-08 mb-15px">
                    <div class="feature-box feature-box-left-icon-middle overflow-hidden">
                        <div class="feature-box-icon feature-box-icon-rounded w-35px h-35px bg-very-light-gray rounded-circle me-15px">
                            <i class="fa-solid fa-check fs-12 text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-500">VPN setup for secure remote access so employees can work from anywhere safely</span>
                        </div>
                    </div>
                </div>
                <!-- end features box item -->
                <!-- start features box item -->
                <div class="icon-with-text-style-08 mb-15px">
                    <div class="feature-box feature-box-left-icon-middle overflow-hidden">
                        <div class="feature-box-icon feature-box-icon-rounded w-35px h-35px bg-very-light-gray rounded-circle me-15px">
                            <i class="fa-solid fa-check fs-12 text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-500">Internet usage policies and website filtering to protect productivity and network safety</span>
                        </div>
                    </div>
                </div>
                <!-- end features box item -->
                <!-- start features box item -->
                <div class="icon-with-text-style-08 mb-15px">
                    <div class="feature-box feature-box-left-icon-middle overflow-hidden">
                        <div class="feature-box-icon feature-box-icon-rounded w-35px h-35px bg-very-light-gray rounded-circle me-15px">
                            <i class="fa-solid fa-check fs-12 text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-500">Continuous monitoring, firmware updates, and security maintenance to keep your firewall current</span>
                        </div>
                    </div>
                </div>
                <!-- end features box item -->
                <!-- start features box item -->
                <div class="icon-with-text-style-08">
                    <div class="feature-box feature-box-left-icon-middle overflow-hidden">
                        <div class="feature-box-icon feature-box-icon-rounded w-35px h-35px bg-very-light-gray rounded-circle me-15px">
                            <i class="fa-solid fa-check fs-12 text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-500">Ongoing IT support for manufacturing companies, corporate offices, medical clinics, and service businesses</span>
                        </div>
                    </div>
                </div>
                <!-- end features box item -->
            </div>
            <div class="col-xl-5 offset-xl-1 col-lg-6 col-md-10" data-anime='{ "el": "childs", "translateY": [30, 0], "opacity": [0,1], "duration": 600, "delay": 100, "staggervalue": 300, "easing": "easeOutQuad" }'>
                <div class="pt-20px pb-20px ps-30px pe-30px border border-color-extra-medium-gray border-radius-6px mb-20px icon-with-text-style-08">
                    <div class="feature-box feature-box-left-icon-middle d-flex align-items-center">
                        <div class="feature-box-icon me-15px">
                            <i class="bi bi-shield-check icon-very-medium text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-600 d-block mb-5px">25+ Years of Experience</span>
                            <p class="fs-13 mb-0">Trusted by businesses across Vadodara and Gujarat for IT infrastructure and managed security services.</p>
                        </div>
                    </div>
                </div>
                <div class="pt-20px pb-20px ps-30px pe-30px border border-color-extra-medium-gray border-radius-6px mb-20px icon-with-text-style-08">
                    <div class="feature-box feature-box-left-icon-middle d-flex align-items-center">
                        <div class="feature-box-icon me-15px">
                            <i class="bi bi-headset icon-very-medium text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-600 d-block mb-5px">Dedicated IT Support</span>
                            <p class="fs-13 mb-0">Our local team in Vadodara is always available to assist with setup, troubleshooting, and ongoing maintenance.</p>
                        </div>
                    </div>
                </div>
                <div class="pt-20px pb-20px ps-30px pe-30px border border-color-extra-medium-gray border-radius-6px icon-with-text-style-08">
                    <div class="feature-box feature-box-left-icon-middle d-flex align-items-center">
                        <div class="feature-box-icon me-15px">
                            <i class="bi bi-building icon-very-medium text-base-color"></i>
                        </div>
                        <div class="feature-box-content">
                            <span class="text-dark-gray fw-600 d-block mb-5px">Built for SMBs</span>
                            <p class="fs-13 mb-0">Enterprise-grade firewall solutions scaled and priced to suit the needs of small and medium businesses.</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- end section -->

<!-- start section - FAQ -->
<section class="bg-very-light-gray pt-5 pb-5">
    <div class="container">
        <div class="row justify-content-center mb-4">
            <div class="col-xl-8 col-lg-9 col-md-10 text-center" data-anime='{ "el": "childs", "translateY": [30, 0], "opacity": [0,1], "duration": 600, "delay": 0, "staggervalue": 300, "easing": "easeOutQuad" }'>
                <span class="ps-25px pe-25px mb-20px text-uppercase text-base-color fs-12 lh-40 fw-700 border-radius-100px bg-gradient-very-light-gray-transparent d-inline-flex justify-content-center"><i class="bi bi-question-circle fs-16 me-5px"></i>FAQ</span>
                <h2 class="text-dark-gray fw-700 ls-minus-1px">Frequently Asked Questions</h2>
            </div>
        </div>
        <div class="row justify-content-center" data-anime='{ "translateY": [30, 0], "opacity": [0,1], "duration": 600, "delay": 0, "staggervalue": 300, "easing": "easeOutQuad" }'>
            <div class="col-xl-8 col-lg-10">
                <div class="accordion accordion-style-02" id="accordion-faq" data-active-icon="icon-feather-minus" data-inactive-icon="icon-feather-plus">

                    <!-- faq item 1 -->
                    <div class="accordion-item">
                        <div class="accordion-header border-bottom border-color-transparent-dark-very-light">
                            <a href="#" data-bs-toggle="collapse" data-bs-target="#faq-01" aria-expanded="false" data-bs-parent="#accordion-faq">
                                <div class="accordion-title mb-0 position-relative text-dark-gray pe-30px">
                                    <i class="feather icon-feather-plus fs-20"></i><span class="fs-17 fw-500">What is a network firewall?</span>
                                </div>
                            </a>
                        </div>
                        <div id="faq-01" class="accordion-collapse collapse" data-bs-parent="#accordion-faq">
                            <div class="accordion-body last-paragraph-no-margin border-bottom border-color-transparent-dark-very-light">
                                <p>A network firewall is a security system that monitors internet traffic entering and leaving your company network. It blocks malicious activity and unauthorized connections while allowing safe, legitimate traffic to pass through — keeping your business data and systems protected at all times.</p>
                            </div>
                        </div>
                    </div>
                    <!-- end faq item 1 -->

                    <!-- faq item 2 -->
                    <div class="accordion-item">
                        <div class="accordion-header border-bottom border-color-transparent-dark-very-light">
                            <a href="#" data-bs-toggle="collapse" data-bs-target="#faq-02" aria-expanded="false" data-bs-parent="#accordion-faq">
                                <div class="accordion-title mb-0 position-relative text-dark-gray pe-30px">
                                    <i class="feather icon-feather-plus fs-20"></i><span class="fs-17 fw-500">Do small businesses really need a firewall?</span>
                                </div>
                            </a>
                        </div>
                        <div id="faq-02" class="accordion-collapse collapse" data-bs-parent="#accordion-faq">
                            <div class="accordion-body last-paragraph-no-margin border-bottom border-color-transparent-dark-very-light">
                                <p>Yes. Small and medium businesses are increasingly targeted by cyber attackers precisely because they often have weaker security systems in place. A professional firewall protects your network from these threats, prevents data breaches, and helps keep your business operations running without interruption.</p>
                            </div>
                        </div>
                    </div>
                    <!-- end faq item 2 -->

                    <!-- faq item 3 -->
                    <div class="accordion-item">
                        <div class="accordion-header border-bottom border-color-transparent-dark-very-light">
                            <a href="#" data-bs-toggle="collapse" data-bs-target="#faq-03" aria-expanded="false" data-bs-parent="#accordion-faq">
                                <div class="accordion-title mb-0 position-relative text-dark-gray pe-30px">
                                    <i class="feather icon-feather-plus fs-20"></i><span class="fs-17 fw-500">Can employees access the office network remotely?</span>
                                </div>
                            </a>
                        </div>
                        <div id="faq-03" class="accordion-collapse collapse" data-bs-parent="#accordion-faq">
                            <div class="accordion-body last-paragraph-no-margin border-bottom border-color-transparent-dark-very-light">
                                <p>Yes. Modern firewalls support secure VPN (Virtual Private Network) connections, which allow employees to safely access office systems, applications, and data from home or while travelling — without exposing the company network to outside threats.</p>
                            </div>
                        </div>
                    </div>
                    <!-- end faq item 3 -->

                    <!-- faq item 4 -->
                    <div class="accordion-item">
                        <div class="accordion-header border-bottom border-color-transparent-dark-very-light">
                            <a href="#" data-bs-toggle="collapse" data-bs-target="#faq-04" aria-expanded="false" data-bs-parent="#accordion-faq">
                                <div class="accordion-title mb-0 position-relative text-dark-gray pe-30px">
                                    <i class="feather icon-feather-plus fs-20"></i><span class="fs-17 fw-500">Will a firewall slow down my internet speed?</span>
                                </div>
                            </a>
                        </div>
                        <div id="faq-04" class="accordion-collapse collapse" data-bs-parent="#accordion-faq">
                            <div class="accordion-body last-paragraph-no-margin border-bottom border-color-transparent-dark-very-light">
                                <p>No. Modern firewall solutions from Fortinet and Sophos are designed to provide high-speed security processing without affecting normal business operations. In fact, features like bandwidth management can actually improve the performance of your most critical business applications.</p>
                            </div>
                        </div>
                    </div>
                    <!-- end faq item 4 -->

                </div>
            </div>
        </div>
    </div>
</section>
<!-- end section -->

<!-- start section - CTA -->
<section class="pt-5 pb-5">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-xl-8 col-lg-10 text-center" data-anime='{ "el": "childs", "translateY": [30, 0], "opacity": [0,1], "duration": 600, "delay": 0, "staggervalue": 300, "easing": "easeOutQuad" }'>
                <span class="ps-25px pe-25px mb-20px text-uppercase text-base-color fs-12 lh-40 fw-700 border-radius-100px bg-gradient-very-light-gray-transparent d-inline-flex justify-content-center"><i class="bi bi-shield-fill-check fs-16 me-5px"></i>Get Protected</span>
                <h2 class="text-dark-gray fw-700 ls-minus-1px">Protect Your Business Network Today</h2>
                <p class="w-80 md-w-100 mx-auto mb-30px">Cyber threats are increasing every year, and businesses in Vadodara and across Gujarat cannot afford to leave their networks unprotected. With professional network firewall services from Nifty Solutions, your company gains a strong security foundation that protects your data, systems, and employees.</p>
                <a href="/contact-us" class="btn btn-medium btn-rounded btn-base-color btn-box-shadow fw-600">Contact Us Today<span class="bg-white text-base-color ms-10px"><i class="feather icon-feather-arrow-right"></i></span></a>
            </div>
        </div>
    </div>
</section>
<!-- end section -->