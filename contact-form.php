...
require __DIR__ . '/config.php'; // for $smtp_credentials and $grecaptcha_secret_key
	...
	if (!empty($grecaptcha_secret_key) && !empty($_POST['g-recaptcha-response'])) {
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if ($enable_smtp == 'no') { // Simple Email
				...
				$mail->setFrom('inquiryform@niftytech.co.in', 'Inquiry Form - Nifty Solutions');
		...
		//Fail Message
		echo '{ "alert": "alert alert-danger alert-dismissable", "message": "Your message could not be sent due to invalid reCaptcha! Token: ' . (isset($_POST['g-recaptcha-response']) ? $_POST['g-recaptcha-response'] : 'MISSING') . '" }';
	}
} else {
	//Empty Email Message
